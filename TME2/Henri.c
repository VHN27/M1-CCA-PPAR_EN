...
int main(int argc, char **argv)
{
    ...
    *displs = 0;
    for (int i = 0; i < p; i++)
    {
        *(sendcounts + i) = nb_pixels + ((i < p - 1) ? 0 : w * h % p);
        if (i > 0)
            *(displs + i) = *(displs + i - 1) + nb_pixels;
    }
    unsigned char *local_image = malloc(*(sendcounts + rank));
    MPI_Scatterv(image, sendcounts, displs, MPI_CHAR, local_image, *(sendcounts + rank), MPI_CHAR, 0, MPI_COMM_WORLD);
    double x = xmin;
    int k = 0;
    for (int i = rank * nb_pixels; i < nb_pixels * (1 + rank) + ((rank == p - 1) ? w * h % p : 0); i++)
    {
        if (i % w == 0 || i == rank * nb_pixels)
        {
            x = xmin;
            y = ymin + yinc * (i / w);
        }
            ...
            x = xmin + xinc * (i % w);
        *(local_image + k++) = xy2color(x, y, depth);
    }
    MPI_Gatherv(local_image, *(sendcounts + rank), MPI_CHAR, image, sendcounts, displs, MPI_CHAR, 0, MPI_COMM_WORLD);
    free(local_image);
    free(sendcounts);
    free(displs);
    /* stop timer */
    ...
    printf("Computing time of process %d: %g sec\n", rank, duration);
    double max_duration;
