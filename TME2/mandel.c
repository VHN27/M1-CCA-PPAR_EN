...
int main(int argc, char **argv)
{
  ...
  int rank, size;
  fprintf(stdout, "Stating MPI_Init\n");
  ...
  fprintf(stdout, "Rank: %d, Size: %d\n", rank, size);

  fprintf(stdout, "Entered 2\n");
  int nb_pixel = w * h / size;
  unsigned char* sub_image = malloc (nb_pixel);
  
  fprintf(stdout, "nb_pixels: %d\n", nb_pixel);
  fprintf(stdout, "Entered 3\n");
  MPI_Scatter (image, nb_pixel, MPI_UNSIGNED_CHAR, sub_image, nb_pixel, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);

  fprintf(stdout, "Entered 4\n");
  double y = ymin ;
  double x = xmin;
  for (int i = 0; i < nb_pixel; i++){
    ...
    fprintf(stdout, "In Process #%d: i = %d, x = %f, y = %f\n",rank, i, x, y);
  }
  ...
  MPI_Gather(sub_image, nb_pixel, MPI_UNSIGNED_CHAR, image, nb_pixel, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);
