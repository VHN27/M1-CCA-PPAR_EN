...
int main(int argc, char **argv)
{
    ...
    double y = ymin;
    int nb_pixels = w * h / p;
    unsigned char *local_image = malloc(nb_pixels);
    MPI_Scatter(image, nb_pixels, MPI_CHAR, local_image, nb_pixels, MPI_CHAR, 0, MPI_COMM_WORLD);

    double x = xmin;
    int k = 0;
    for (int i = rank * nb_pixels; i < nb_pixels * (1 + rank); i++)
    {
        if (i % w == 0)
        {
            x = xmin;
    ...
    fprintf(stdout, "Size of sub_image in process #%d is: %ld\n", rank, strlen(local_image));

    MPI_Gather(local_image, nb_pixels, MPI_CHAR, image, nb_pixels, MPI_CHAR, 0, MPI_COMM_WORLD);
    /* stop timer */
